...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    ...
    >
      <Link href={`/product/${product.id}`} className="block relative aspect-[3/4] overflow-hidden bg-stone-100 mb-4">
        {product.isNew && (
          <span className="absolute top-3 left-3 bg-white/95 backdrop-blur px-2 py-1 text-[10px] uppercase tracking-widest font-bold z-10 text-stone-900 shadow-sm">
            Nieuw
          </span>
        )}
        {product.isBestseller && (
          <span className="absolute top-3 right-3 bg-stone-900 text-white px-2 py-1 text-[10px] uppercase tracking-widest font-bold z-10 shadow-sm">
            Bestseller
          </span>
        )}
        <img
